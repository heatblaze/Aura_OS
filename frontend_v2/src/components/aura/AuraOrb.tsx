...
const STATE_CONFIG = {
  idle:      { color1: "#00d4ff", color2: "#8b5cf6", color3: "#3b82f6", glow: 0.18, speed: 0.008, waves: 5, amp: 0.12, ringSpeed: 25 },
  listening: { color1: "#00d4ff", color2: "#06b6d4", color3: "#22d3ee", glow: 0.35, speed: 0.016, waves: 7, amp: 0.22, ringSpeed: 15 },
  thinking:  { color1: "#8b5cf6", color2: "#a855f7", color3: "#7c3aed", glow: 0.28, speed: 0.025, waves: 8, amp: 0.15, ringSpeed: 8 },
  speaking:  { color1: "#00d4ff", color2: "#ffffff", color3: "#8b5cf6", glow: 0.4,  speed: 0.02,  waves: 6, amp: 0.3,  ringSpeed: 12 },
} as const;

/* ══════════════════════════════════════════════
   CANVAS ORB — Dynamic energy waves inside sphere
══════════════════════════════════════════════ */
function EnergyCanvas({ size, state, amplitude }: { size: number; state: VisualizerState; amplitude: number }) {
  ...
  const draw = useCallback(() => {
    ...
    for (let w = 0; w < waveCount; w++) {
      ...
      grad.addColorStop(0.2, w % 2 === 0 ? cfg.color1 : cfg.color2);
      grad.addColorStop(0.5, cfg.color3);
      grad.addColorStop(0.8, w % 2 === 0 ? cfg.color2 : cfg.color1);
    ...
    innerGlow.addColorStop(0, `${cfg.color1}30`);
    innerGlow.addColorStop(0.5, `${cfg.color1}10`);
...
/* ══════════════════════════════════════════════
   MAIN ORB COMPONENT
══════════════════════════════════════════════ */
export function AuraOrb({ state, amplitude = 0.5, size = 300 }: AuraOrbProps) {
  const cfg = STATE_CONFIG[state];
    ...
    <div style={{ width: size, height: size, position: "relative" }} className="select-none">
      ...
      <motion.div
        style={{
          ...
          background: `radial-gradient(circle, ${cfg.color1} 0%, ${cfg.color2}40 30%, transparent 65%)`,
      ...
      {/* Orbital rings */}
      <OrbitalRing size={size} radius={size * 0.52} speed={cfg.ringSpeed} color={cfg.color1} tilt={65} width={1.2} />
      <OrbitalRing size={size} radius={size * 0.48} speed={cfg.ringSpeed * 1.4} color={cfg.color2} tilt={72} width={0.8} />
      <OrbitalRing size={size} radius={size * 0.56} speed={cfg.ringSpeed * 0.7} color={`${cfg.color1}60`} tilt={58} width={0.6} />

      {/* Orbit particles */}
      {[0, 1, 2, 3, 4, 5].map(i => (
        <OrbitParticle key={i} size={size} radius={size * (0.48 + (i % 3) * 0.04)}
          speed={cfg.ringSpeed * (0.8 + i * 0.15)} delay={i * 1.5} color={i % 2 === 0 ? cfg.color1 : cfg.color2}
          particleSize={2 + (i % 3)} />
      ))}

      {/* Canvas energy waves */}
      <EnergyCanvas size={size} state={state} amplitude={amplitude} />
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
        <motion.div
          style={{
            ...
            background: "white",
            boxShadow: `0 0 16px 4px white, 0 0 40px 10px ${cfg.color1}80, 0 0 80px 20px ${cfg.color1}30`,
          }}
          animate={{
            scale: [1, 1.15, 1],
            boxShadow: [
              `0 0 16px 4px white, 0 0 40px 10px ${cfg.color1}60, 0 0 80px 20px ${cfg.color1}20`,
              `0 0 24px 8px white, 0 0 60px 16px ${cfg.color1}90, 0 0 100px 30px ${cfg.color1}40`,
              `0 0 16px 4px white, 0 0 40px 10px ${cfg.color1}60, 0 0 80px 20px ${cfg.color1}20`,
            ],
          }}
      ...
      <AnimatePresence>
        {(state === "listening" || state === "speaking") && [0, 0.8, 1.6].map((delay, i) => (
          <motion.div key={i} style={{
            ...
            borderRadius: "50%", border: `1px solid ${cfg.color1}`, x: "-50%", y: "-50%",
          }}
